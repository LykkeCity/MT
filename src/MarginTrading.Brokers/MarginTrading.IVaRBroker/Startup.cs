    ...
    public class Startup
    {
        ...
        private void RegisterRepositories(ContainerBuilder builder, MarginSettings settings)
        {
            LykkeLogToAzureStorage log = new LykkeLogToAzureStorage(PlatformServices.Default.Application.ApplicationName,
                new AzureTableStorage<LogEntity>(settings.Db.LogsConnString, "MarginTradingIVaRBrokerLog", null));
            ...
            builder.Register<IMarginTradingIndividualValuesAtRiskRepository>(ctx =>
                AzureRepoFactories.MarginTrading.CreateIndividualValuesAtRiskRepository(settings.Db.MarginTradingConnString, log)
            ).SingleInstance();
