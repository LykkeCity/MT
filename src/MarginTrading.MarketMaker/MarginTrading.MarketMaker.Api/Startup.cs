...
using MarginTrading.MarketMaker.Api.Modules;
...
namespace MarginTrading.MarketMaker.Api
{
    public class Startup
    {
        ...
        public IServiceProvider ConfigureServices(IServiceCollection services)
        {
            ...
            {
                options.DefaultLykkeConfiguration("v1", "MarginTrading_MarketMaker_Api API");
            });
            ...
            builder.RegisterModule(new ServiceModule(appSettings.MarginTrading_MarketMaker_ApiService, log));
        ...
        public void Configure(IApplicationBuilder app, IHostingEnvironment env, IApplicationLifetime appLifetime)
        {
            ...
            app.UseLykkeMiddleware("MarginTrading_MarketMaker_Api", ex => new {Message = "Technical problem"});
        ...
        private static ILog CreateLogWithSlack(IServiceCollection services, AppSettings settings)
        {
            ...
            var dbLogConnectionString = settings.MarginTrading_MarketMaker_ApiService.Db.LogsConnString;

            // Creating azure storage logger, which logs own messages to concole log
            if (!string.IsNullOrEmpty(dbLogConnectionString) && !(dbLogConnectionString.StartsWith("${") && dbLogConnectionString.EndsWith("}")))
            {
                logToAzureStorage = new LykkeLogToAzureStorage("Lykke.Service.MarginTrading_MarketMaker_Api", new AzureTableStorage<LogEntity>(
                    dbLogConnectionString, "MarginTrading_MarketMaker_ApiLog", logToConsole));
